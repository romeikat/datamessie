...
import com.romeikat.datamessie.model.callback.SearchForCleanedContentCallback;
...
public class DocumentsFilterSettings implements Serializable, Cloneable {
  ...
  public void transformCleanedContentIntoDocumentIds(
      final SearchForCleanedContentCallback searchForCleanedContentCallback) {
    // Modify in order to determine document IDs only once
    if (Strings.isNullOrEmpty(cleanedContent)) {
      return;
    }

    final Collection<Long> documentIds =
        searchForCleanedContentCallback.getDocumentIdsWithCleanedContent(cleanedContent);
