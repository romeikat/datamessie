/**
 * Workaround to handle German umlaute in date fields.
 */
/*
...
public class DateParser {
  ...
  private static final String[] W3CDATETIME_MASKS =
      {"yyyy-MM-dd'T'HH:mm:ss.SSSz", "yyyy-MM-dd't'HH:mm:ss.SSSz", "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'",
          ...
          "yyyy-MM-dd't'HH:mm:ss'z'", "yyyy-MM-dd'T'HH:mmz", // together
          // with
          // logic
          // in
          // the
   ...
   * forgiving than what the Atom spec allows. The forms that are invalid according to the spec are
   * indicated.
   */
  ...
  private static String convertUnsupportedTimeZones(final String sDate) {
    final List<String> unsupportedZeroOffsetTimeZones = Arrays.asList("UT", "Z");

    for (final String timeZone : unsupportedZeroOffsetTimeZones) {
  ...
  private static String replaceLastOccurrence(final String original, final String target,
    ...
    if (lastIndexOfTarget == -1) {
      ...
      return new StringBuilder(original)
          .replace(lastIndexOfTarget, lastIndexOfTarget + target.length(), replacement).toString();
    }
  }
   ...
   *
   */
  public static Date parseDate(String sDate, final Locale locale) {
    // BEGIN WORKAROUND
    // Replace German umlaute in order to avoid parsing issues
    sDate = sDate.replace('?', 'a').replace('?', 'o').replace('?', 'u');
    // END WORKAROUND
   ...
   * @return the RFC822 represented by the given Date It returns <b>null</b> if it was not possible
   *         to parse the date.
   *
   */
